...
describe('WebhookHandler', () => {
  ...
  it('should verify ISV webhook with valid signature', () => {
    const body = JSON.stringify({
      paymentRequestId: 'REQ123', status: 'SUCCESS', amount: '100.00',
      currency: 'USD', tradeTime: '2024-01-01T00:00:00Z', message: '支付成功',
    });
    const timestamp = '1700000000000';
    const sig = sign(WEBHOOK_SECRET, 'POST', NOTIFY_URL, timestamp, body);

    const event = handler.handle(
      { 'X-Isv-Signature': sig, 'X-Isv-Timestamp': timestamp }, body, NOTIFY_URL,
    );
  ...
  it('should reject ISV webhook with invalid signature', () => {
    expect(() =>
      handler.handle(
        ...
        '{"paymentRequestId":"REQ123"}', NOTIFY_URL,
      ),
  ...
  it('should handle notification sign.success event', () => {
    const body = JSON.stringify({
      type: 'sign.success', referenceMerchantId: 'M001', message: '签约成功', timestamp: '1700000000000',
    });
    const timestamp = '1700000000000';
    const sig = sign(WEBHOOK_SECRET, 'POST', NOTIFY_URL, timestamp, body);

    const event = handler.handle(
      { 'X-Webhook-Signature': sig, 'X-Webhook-Timestamp': timestamp }, body, NOTIFY_URL,
    );
