...
class PaygateClientIT {
    ...
    void shouldCallPayAndParseResponse() throws IOException {
        server.createContext("/pay", exchange -> {
            byte[] resp = "{\"code\":\"0\",\"message\":\"ok\",\"redirectUrl\":\"https://checkout.example.com/abc\",\"paymentRequestId\":\"REQ001\"}"
    ...
    void shouldSignRequestCorrectly() throws IOException {
        ...
        server.createContext("/pay", exchange -> {
            ...
            byte[] resp = "{\"code\":\"0\",\"message\":\"ok\",\"redirectUrl\":\"https://x.com\",\"paymentRequestId\":\"X\"}"
                    .getBytes(StandardCharsets.UTF_8);
            exchange.sendResponseHeaders(200, resp.length);
            exchange.getResponseBody().write(resp);
            exchange.close();
